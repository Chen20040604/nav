...
namespace IG_LIO
{
    ...
    public:
        ...
        void operator()()
        {
            ...
            rate_ = std::make_shared<rclcpp::Rate>(1.0);
            while (is_alive)
            {
                rate_->sleep();
                if (terminate_flag)
                ...
                if (rectify)
                {
                    Eigen::Matrix3d new_offset_rot = gloabl_pose_.block<3, 3>(0, 0) * local_rot_.transpose();
                    Eigen::Vector3d new_offset_pos = -gloabl_pose_.block<3, 3>(0, 0) * local_rot_.transpose() * local_pos_ + gloabl_pose_.block<3, 1>(0, 3);
                    double diff = (new_offset_pos - shared_data_->offset_pos).norm();
                    double new_yaw = atan2(new_offset_rot(1, 0), new_offset_rot(0, 0));
                    double old_yaw = atan2(shared_data_->offset_rot(1, 0), shared_data_->offset_rot(0, 0));
                    double yaw_diff = std::fabs(new_yaw - old_yaw);
                    if ((diff < dis_thresh_ && yaw_diff < yaw_thresh_) || shared_data_->localizer_service_called)
                    {
                        std::lock_guard<std::mutex> lock(shared_data_->mutex);
